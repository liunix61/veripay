...
## 组合叙事（VeriAgent × VeriPay）

VeriAgent 审计 Agent 的每一次投资决策（四哈希凭证+bStocks/RWA 合规+Jev/NanoJev 双轨决策模型审计），VeriPay 审计 Agent 的每一笔付费支出（EIP-712 授权+四项硬限额+链上收据）——AI Agent 金融活动的资金两端全部链上可验证。决策侧凭证与支付侧收据共用同一设计哲学：约束进合约、证据进链、顺序可独立验证。详见 `docs/07-组合叙事-veriagent-x-veripay.md`（中英双语）。
